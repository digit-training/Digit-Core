...
export const UICustomizations = {
  ...
  SearchOrganisationConfig: {
    preProcess: (data) => {
      const createdFromEpoch = new Date(data.body.SearchCriteria.createdFrom).getTime();
      const createdToEpoch = new Date(data.body.SearchCriteria.createdTo).getTime();

      data.body.SearchCriteria = {
        ...data.body.SearchCriteria,
        createdFrom: createdFromEpoch,
        createdTo: createdToEpoch,
        tenantId: Digit.ULBService.getCurrentTenantId()
      };

      var boundaryCode = "B";
      if (data?.body?.SearchCriteria?.boundaryCode != undefined) {
