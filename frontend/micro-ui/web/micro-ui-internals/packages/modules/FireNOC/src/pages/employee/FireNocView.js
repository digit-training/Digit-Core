...
const FireNocView = () => {
  ...
  // const tenantId = Digit.ULBService.getCurrentTenantId();
  const {applicationNumber, tenantId } = Digit.Hooks.useQueryParams();

  const payload = {
    FireNOCs: { tenantId: tenantId },
  };
  const searchParams = {
    offset: 0,
    limit: 100,
  ...
  const { isLoading, data, isError, isSuccess, error } = Digit.Hooks.useViewFirenoc({
    tenantId,
    data: payload,
    searchParams,
    config: { cacheTime: 0 },
  });
  console.log(data, "data will show jhere");
  ...
  return (
    <React.Fragment>
      <Header className="works-header-view">FireNOC Details</Header>
      {
